...
## [10] Кнопкове меню — handlers/menu.py

`ReplyKeyboardMarkup` (`menu.MAIN_KEYBOARD`, `resize_keyboard=True`),
надсилається один раз при `/start`/`/help` — Telegram-клієнт тримає
кнопки внизу екрана постійно, без повторної відправки. Кожна кнопка
дублює команду (`🌅 Прев'ю зараз` → `/support`, `🌴`/`💼` → `/markweekend`
`/markworkday` **на сьогодні** — інша дата й далі лише командою з
аргументом, `📝` → `/custom`, `❌` → `/cancel`).

`menu.on_button` — окремий `MessageHandler(filters.Text(BUTTON_LABELS))`,
зареєстрований **перед** загальним `custom.on_text` (у межах групи PTB
зупиняється на першому хендлері, що збігся, — конфлікту з активним
`/custom` вводом нема). Для будь-якої кнопки, крім «Своє повідомлення»
і «Скасувати», спершу викликає `custom.reset_state` — інакше
