...
За замовчуванням `AUTO_PUBLISH_ENABLED=true`: о 7:30, якщо прев'ю по
важкій ночі без реакції жодного адміна, бот **сам публікує**
повідомлення в General. Спокійна ніч без реакції — тиша.

Повернути ручний режим тестового періоду (лише нагадування адмінам):
...
**Прев'ю приходить, але о 7:30 нічого не публікується без натискань.**
Автопублікація спрацьовує лише для ночей, які алгоритм визнав важкими
(рядок-рекомендація в прев'ю). Якщо ніч важка — перевір, що в
`/etc/vb-assistant-bot/env` не стоїть `AUTO_PUBLISH_ENABLED=false`
(старі інсталяції тестового періоду) — див. розділ вище.
