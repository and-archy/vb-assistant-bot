...
`AUTO_PUBLISH_ENABLED=false` за замовчуванням: о 7:30 (`autopublish_time`
у `config/thresholds.json`) без реакції на прев'ю по важкій ночі бот
лише нагадує адмінам, ніколи не публікує сам. Свідоме рішення ТЗ —
перевірити точність тригера, перш ніж довіряти автопублікацію.
